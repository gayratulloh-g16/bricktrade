<x-layouts.driver>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('driver.orders.update', $order->id) }}" method="POST" class="row g-3">
                                @csrf
                                @method('PUT')
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" step="0.01" class="form-control" id="total_amount" name="total_amount" value="{{ old('total_amount', $order->total_amount) }}" readonly>
                                    @error('total_amount')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <table class="table" id="order-items-table">
                                        ...
                                        <tbody>
                                            @php
                                                // Use old input if available, otherwise use the order's existing items
                                                $orderItems = old('order_items', $order->orderItems->toArray());
                                            @endphp
                                                    ...
                                                    <td>
                                                        <select name="order_items[{{ $index }}][brick_id]" class="form-select brick-select" required>
                                                            ...
                                                            @foreach($bricks as $brick)
                                                                <option value="{{ $brick->id }}" {{ $item['brick_id'] == $brick->id ? 'selected' : '' }}>
                                                                    {{ $brick->name_en }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error("order_items.$index.brick_id")
                                                            <small class="text-danger">{{ $message }}</small>
                                                        @enderror
                                                    </td>
                                                    <td>
                                                        <input type="number" name="order_items[{{ $index }}][quantity]" class="form-control quantity" value="{{ $item['quantity'] }}" required>
                                                        @error("order_items.$index.quantity")
                                                            <small class="text-danger">{{ $message }}</small>
                                                        @enderror
                                                    </td>
                                                    <td>
                                                        <input type="number" step="0.01" name="order_items[{{ $index }}][unit_price]" class="form-control unit-price" value="{{ $item['unit_price'] }}" required>
                                                        @error("order_items.$index.unit_price")
                                                            <small class="text-danger">{{ $message }}</small>
                                                        @enderror
                                                    </td>
                                                    <td>
                                ...
                                <div class="col-12">
                                    ...
                                    <a href="{{ route('driver.orders.index') }}" class="btn btn-secondary">Cancel</a>
                                </div>
                            </form>
                        </div><!-- End Card Body -->
                    </div><!-- End Card -->
    ...
    <script>
        ...
        // Attach events to any existing rows on page load
        document.querySelectorAll('#order-items-table tbody tr').forEach(function(row) {
            attachEvents(row);
        });
